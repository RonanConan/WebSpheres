...
var _default = AFRAME.registerComponent("hand-tracking-extras", {
  ...
  tick: function () {
    return function () {
      ...
      // --- *** FIX 2: ROBUST COMPONENT CHECK *** ---
      var handControls = this.el.components['hand-tracking-controls'];
      
      // Wait for handControls AND its internal trackedControls to be initialized
      // (handControls.trackedControls is set in hand-tracking-controls.init)
      if (!handControls || !handControls.trackedControls) {
        return; 
      }
      ...
      if (!trackedControlsWebXR) {
        return; // Should be redundant, but safe
      }
      ...
      if (!controller || !frame || !referenceSpace) {
        // controllerFound may still be null for a few frames
        return;
      }
