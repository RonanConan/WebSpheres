...
var RayCurve = function (numPoints, width) {
  ...
  this.vertices = new Float3TArray(numPoints * 3 * 2);
  this.uvs = new Float3TArray(numPoints * 2 * 2);
  this.width = width;
...
var _default = AFRAME.registerComponent("hand-tracking-extras", {
  ...
  tick: function () {
    return function () {
      ...
      // --- CRITICAL FIX START ---
      // 1. Get the main A-Frame hand-tracking component
      var handControls = this.el.components['hand-tracking-controls'];
      if (!handControls) return; 

      // 2. Extract the controller from the internal properties
      var trackedControlsWebXR = handControls.trackedControls;
      // Use 'controllerFound' (A-Frame 1.7.0+) or fallback to 'controller'
      var controller = handControls.controllerFound || (trackedControlsWebXR && trackedControlsWebXR.controller);
